...
For a visually authored asset slice, independently inspect the changed asset, original sample, and every selected style-cohort member named in the implementer's report. If the changed asset, identity reference, or a selected cohort member is inaccessible, send the PR back before Review; a legitimately small cohort is not a missing reference. For a mechanical-only asset slice, independently verify rendered-pixel equivalence; a visual difference reclassifies it as visually authored and requires the reference set. The review agent needs the actual pixels, not a prose claim about them.

### 4. Review

After Verify passes, the **orchestrator** runs `/code-review` on the PR branch — not the implementer, and not as a substitute for the acceptance-criteria gate above. Fixed point is `main` (`git diff main...HEAD`); Spec source is the live issue body (already fetched for Verify). Follow the `code-review` skill: Standards and Spec as parallel sub-agents, reports kept under separate headings. For a visually authored asset slice, pass the slice type and verified visual reference set into `/code-review`; the Spec reviewer must return the per-asset Style alignment / Identity continuity verdict table, and every `fail` or `unverified` row is a blocking Spec finding. For a mechanical-only asset slice, pass the verified pixel-equivalence evidence instead; it needs no identity/style verdict unless the rendered pixels differ.
