...
In the repo: `main` clean (`git status --porcelain` empty), up to date, full test suite green, latest CI run on `main` green. Read the wave's issues and their native `blocked_by` edges, plus any dispatch order recorded by `/plan-wave`. Require every issue to contain `## Slice type`, `## Delta`, `## Contract`, `## Touches`, `## Proof`, and `## Blocked by`; `## Invariants` is optional. Stop and ask when a required section is missing. Resolve each issue's `## Touches` anchors before dispatch — mechanically where the repo ships an anchor extractor (in Underline, `npm run agents:anchors -- --issue <N>`), by grep otherwise. Dispatch is the last point where a manifest can be repaired cheaply: an anchor that resolves to nothing sends its implementer back to reading whole modules, which it then re-sends on every round trip for the rest of the slice. A handful of unresolvable prose anchors is a note in the envelope; a manifest that mostly does not resolve goes back to `/plan-wave` for repair. Record each issue's slice type on the dispatch plan. Treat each Contract claim as a **completion claim** and its Proof mapping as the required evidence. Classify each asset slice as visually authored or mechanical-only from the issue: a mechanical-only slice must prove rendered-pixel equivalence; any intended visual delta makes it visually authored. For every visually authored asset slice, build the visual reference set before dispatch: the issue's original sample plus two to five closest existing assets from the same family, atlas, role, or rendered context; when fewer than two peers exist, use every available peer and say so. If the original sample is unavailable, or no existing asset can establish the requested style, stop and ask rather than dispatching an identity- or style-blind generation. An issue is **parallel-safe** only when it is both file-disjoint *and* semantically disjoint from everything in flight. Compute file-disjointness mechanically: intersect the issues' `## Touches` write sets (`modify` + `create`) — any overlap is a conflict. Semantic disjointness still needs judgment — slices can collide through shared fixtures, test conditions, or event timing even when their manifests never overlap. Writers of the same core file are strictly serial. When in doubt, serialize.
